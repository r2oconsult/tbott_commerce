  ...
  async function enviarPergunta() {
    ...
    try {
      const res = await fetch('/api/tbott', {
        ...
        body: JSON.stringify({ pergunta, titulo: produto.titulo, autor: produto.autor, descricao: produto.descricao, categoria: produto.categoria }),
      });
  ...
  return (
      ...
      <div className='overflow-hidden transition-all duration-300' style={{ maxHeight: hover || tbottAberto ? '700px' : '0px', opacity: hover || tbottAberto ? 1 : 0 }}>
        ...
        <div className='flex flex-col gap-2 mb-4'>
          <div className='flex justify-between text-sm'><span className='text-neutral-500'>Capa Mole</span><span className='text-neutral-800 font-medium'>R$ {produto.preco_base.toFixed(2).replace('.', ',')}</span></div>
          <div className='flex justify-between text-sm'><span className='text-neutral-500'>Capa Dura Premium</span><span className='text-neutral-800 font-medium'>R$ {produto.preco_capa_dura.toFixed(2).replace('.', ',')}</span></div>
        </div>
